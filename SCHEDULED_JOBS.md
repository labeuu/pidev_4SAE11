...
This document describes the **automatic background tasks** that run inside the **Planning** and **Task** microservices. They use Spring’s `@Scheduled` feature and the **cron** expressions loaded from the **Config Server** (`planning.properties` and `task.properties`). Local `application.properties` in each microservice only bootstraps the Config client (same pattern as Offer).
...
| **Daily overdue reminders** | **Every day at 08:00:00** (`0 0 8 * * ?`) | Sends in-app reminders to freelancers with overdue assigned tasks or subtasks. | **Yes** (via Notification service) |

**Config properties (Task):**

- `task.scheduler.escalate-cron` — priority escalation job  
- `task.scheduler.purge-cron` — purge job  
- `task.scheduler.overdue-reminder-cron` — daily overdue reminder job  
...
2. Optionally override locally with **`application-local.properties`** in the microservice (gitignored) if you use that file for secrets or machine-specific settings.  
